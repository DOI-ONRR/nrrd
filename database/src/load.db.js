...
const main = async () => {
  ...
  etl.file(CSV)
    .pipe(etl.csv()).pipe(etl.collect(BATCH)).promise().then(async data => {
	  ...
	  const json_row = JSON.parse(string)
  ...
  for (const field in row) {
    ...
    case 'Commodity':
      ...
      if (tmp.match(/Prod Vol/)) {
        const tmp1 = tmp.replace(' Prod Vol ', '|')
        const a = tmp1.split('|')
...
const addLocation = async (row, lookup) => {
  ...
  for (const field in row) {
    ...
    case 'Land Category':
	  ...
	  row['Revenue Type'] === 'Other Revenues') {
        if (row['Land Class'] !== DFC.NATIVE_AMERICAN &&
	    !row['Offshore Region']) {
  ...
  const state_name = STATE_NAME_MAP[state] || ''
...
const addCommodity = async (row, lookup) => {
  ...
  if (revenue_category === undefined && disbursement_category == '') {
    if (row['Land Class'] === DFC.NATIVE_AMERICAN) {
      revenue_category = DFC.NATIVE_AMERICAN
    }
    else {
...
const LAND_CATEGORY_TO_DISPLAY_NAME = {
  Offshore: DFC.OFFSHORE_CAPITALIZED,
  Onshore: DFC.ONSHORE,
}

const LAND_CLASS_TO_DISPLAY_NAME = {
  Federal: DFC.FEDERAL,
  'Native American': DFC.NATIVE_AMERICAN,
}

const LAND_CLASS_CATEGORY_TO_REVENUE_CATEGORY = {
  [DFC.FEDERAL]: {
    [DFC.OFFSHORE_CAPITALIZED]: DFC.FEDERAL_OFFSHORE,
    [DFC.ONSHORE]: DFC.FEDERAL_ONSHORE,
  },
  [DFC.NATIVE_AMERICAN]: {
    [DFC.OFFSHORE_CAPITALIZED]: DFC.NATIVE_AMERICAN,
    [DFC.ONSHORE]: DFC.NATIVE_AMERICAN,
  },
}
