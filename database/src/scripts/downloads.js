...
const metaData = (data_set, extension, period) => {
	...
	res => {
	    let min_period=new Date(res.rows[0].min_period)
	    let max_period=new Date(res.rows[0].max_period)
	    ...
	    var csv_stats = fs.statSync("/tmp/downloads/"+data_set+"."+extension)
...
const processFiles = async () => {
    console.log('start')
    let datasets=fs.readdirSync('/tmp/downloads/');
    console.log('datasets', datasets)
    for(let ii=0; ii<datasets.length; ii++) {
	let [dataset, extension]=datasets[ii].split('.')
...
const main = async () => {
    ...
    let data = JSON.stringify(metaJson);
