...
union
(select 'National' as qland_category,  fiscal_year, 'National' as state_or_area, sum(revenue), count(distinct(commodity)) as distinct_commodities 
from revenue natural join period natural join location natural join commodity 
where period='Fiscal Year'
group by state_or_area, fiscal_year order by fiscal_year, state_or_area
)
;`
}
...
const revenue_commodity_summary =() => {
    let view=`
drop view revenue_commodity_summary;
...
union
(select 'National' as state_or_area  ,fiscal_year, commodity, sum(revenue) as total from revenue natural join location natural join period natural join  commodity  where period='Fiscal Year' group by state_or_area, fiscal_year, commodity order by fiscal_year, state_or_area, total desc)
;
...
union
(select 'National' as state_or_area  ,fiscal_year,  sum(revenue) as total from revenue natural join location natural join period natural join  commodity  where period='Fiscal Year' group by state_or_area, fiscal_year, commodity order by fiscal_year, state_or_area, total desc)
