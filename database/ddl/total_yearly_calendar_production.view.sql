...
--
-- Name: total_yearly_calendar_production; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.total_yearly_calendar_production AS
 SELECT b.year,
    b.month,
    b.month_long,
    b.product_order,
           ...
           FROM ( SELECT t1.period,
                   ...
                   FROM (( SELECT period.period,
                          ...
                          WHERE (((period.period)::text = 'Monthly'::text) AND (period.calendar_year <= ( SELECT max(period_1.calendar_year) AS max
                                  ...
                                  WHERE (period_1.month = 1))) AND (period.calendar_year > ( SELECT (max(period_1.calendar_year) - 11)
                                   FROM (public.production production_1
                                     JOIN public.period period_1 USING (period_id))
                                  WHERE (period_1.month = 12))) AND ((commodity.product)::text = ANY (ARRAY[('Oil (bbl)'::character varying)::text, ('Gas (mcf)'::character varying)::text, ('Coal (tons)'::character varying)::text])))
                          GROUP BY period.period, period.calendar_year, period.month, period.month_long, location.land_type, location.land_category, location.land_class, commodity.product,
                                CASE
                                    WHEN ((commodity.product)::text = 'Oil (bbl)'::text) THEN 1
                                    WHEN ((commodity.product)::text = 'Gas (mcf)'::text) THEN 2
                                    WHEN ((commodity.product)::text = 'Coal (tons)'::text) THEN 3
                     ...
                     LEFT JOIN ( SELECT DISTINCT period.period,
                          ...
                          WHERE (((period.period)::text = 'Monthly'::text) AND (period.calendar_year <= ( SELECT max(period_1.calendar_year) AS max
                                  ...
                                  WHERE (period_1.month = 1))) AND (period.calendar_year > ( SELECT (max(period_1.calendar_year) - 11)
                                   FROM (public.production production_1
                                     JOIN public.period period_1 USING (period_id))
                                  WHERE (period_1.month = 12))) AND ((commodity.product)::text = ANY (ARRAY[('Oil (bbl)'::character varying)::text, ('Gas (mcf)'::character varying)::text, ('Coal (tons)'::character varying)::text])))) t2 USING (period, year))) a
...
ALTER TABLE public.total_yearly_calendar_production OWNER TO postgres;
