...
--
-- Name: total_monthly_last_three_years_disbursement; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.total_monthly_last_three_years_disbursement AS
 SELECT period.period,
  ...
  GROUP BY period.period, period.calendar_year, period.month, period.month_long, period.period_date, location.land_type, fund.fund_class, fund.recipient,
        CASE
            WHEN ((location.land_type)::text = 'Native American'::text) THEN 2
            WHEN ((location.land_type)::text = 'Federal onshore'::text) THEN 4
            WHEN ((location.land_type)::text = 'Federal offshore'::text) THEN 3
...
ALTER TABLE public.total_monthly_last_three_years_disbursement OWNER TO postgres;
