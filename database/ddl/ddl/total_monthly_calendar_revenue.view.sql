...
--
-- Name: total_monthly_calendar_revenue; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.total_monthly_calendar_revenue AS
 SELECT period.period,
  ...
  WHERE (((period.period)::text = 'Monthly'::text) AND (period.calendar_year <= ( SELECT max(period_1.calendar_year) AS max
          ...
          WHERE (period_1.month = 12))) AND (period.calendar_year > ( SELECT (max(period_1.calendar_year) - 2)
           FROM (public.revenue revenue_1
             JOIN public.period period_1 USING (period_id))
...
ALTER TABLE public.total_monthly_calendar_revenue OWNER TO postgres;
