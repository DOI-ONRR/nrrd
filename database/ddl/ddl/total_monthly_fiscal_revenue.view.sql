...
--
-- Name: total_monthly_fiscal_revenue; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.total_monthly_fiscal_revenue AS
 SELECT period.period,
  ...
  WHERE (((period.period)::text = 'Monthly'::text) AND (period.fiscal_year <= ( SELECT max(period_1.fiscal_year) AS max
          ...
          WHERE (period_1.fiscal_month = 12))) AND (period.fiscal_year > ( SELECT (max(period_1.fiscal_year) - 3)
           FROM (public.revenue revenue_1
             JOIN public.period period_1 USING (period_id))
          WHERE (period_1.fiscal_month = 12))))
  GROUP BY period.period, period.fiscal_year, period.fiscal_month, period.month_long, period.period_date, commodity.commodity, commodity.commodity_order, fund.revenue_type, location.land_type,
        CASE
            WHEN ((location.land_type)::text = 'Native American'::text) THEN 2
            WHEN ((location.land_type)::text = 'Federal - not tied to a lease'::text) THEN 0
            WHEN ((location.land_type)::text = 'Federal - Not tied to a location'::text) THEN 1
...
ALTER TABLE public.total_monthly_fiscal_revenue OWNER TO postgres;
