...
--
-- Name: total_monthly_last_three_years_revenue; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.total_monthly_last_three_years_revenue AS
 SELECT period.period,
  ...
  WHERE (((period.period)::text = 'Monthly'::text) AND (period.period_date > ( SELECT (max(period_1.period_date) - '3 years'::interval)
           FROM (public.revenue revenue_1
             JOIN public.period period_1 USING (period_id))
          WHERE ((period_1.period)::text = 'Monthly'::text))))
  GROUP BY period.period, period.calendar_year, period.month, period.month_long, period.period_date, commodity.commodity, commodity.commodity_order, fund.revenue_type, location.land_type,
        CASE
            WHEN ((location.land_type)::text = 'Native American'::text) THEN 2
            WHEN ((location.land_type)::text = 'Federal - not tied to a lease'::text) THEN 0
            WHEN ((location.land_type)::text = 'Federal - Not tied to a location'::text) THEN 1
...
ALTER TABLE public.total_monthly_last_three_years_revenue OWNER TO postgres;
