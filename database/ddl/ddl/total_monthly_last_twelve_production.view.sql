...
--
-- Name: total_monthly_last_twelve_production; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.total_monthly_last_twelve_production AS
 SELECT a.period,
    a.year,
    a.month,
    a.month_long,
   ...
   FROM ( SELECT t1.period,
           ...
           FROM (( SELECT period.period,
                  ...
                  GROUP BY period.period, period.calendar_year, period.month, period.month_long, period.period_date, commodity.product, location.land_category, location.land_class,
                        CASE
                            WHEN ((location.land_class)::text = 'Mixed Exploratory'::text) THEN 0
                            WHEN (((location.land_class)::text = 'Native American'::text) OR ((location.land_class)::text = 'Native American '::text)) THEN 1
                            WHEN (((location.land_class)::text = 'Federal'::text) AND ((location.land_category)::text = 'Onshore'::text)) THEN 3
...
ALTER TABLE public.total_monthly_last_twelve_production OWNER TO postgres;
