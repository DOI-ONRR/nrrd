...
--
-- Name: total_monthly_calendar_production; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.total_monthly_calendar_production AS
 SELECT a.period,
    a.year,
    a.month,
    a.month_long,
   ...
   FROM ( SELECT t1.period,
            ...
            t2.sort_order,
                CASE
                    WHEN ((t1.source)::text = t2.source) THEN t1.sum
                    ELSE NULL::numeric
                END AS sum
           FROM (( SELECT period.period,
                  ...
                  WHERE (((period.period)::text = 'Monthly'::text) AND (period.calendar_year <= ( SELECT max(period_1.calendar_year) AS max
                          ...
                          WHERE (period_1.month = 12))) AND (period.calendar_year >= ( SELECT max((period_1.calendar_year - 1)) AS max
                           FROM (public.production production_1
                             JOIN public.period period_1 USING (period_id))
                          WHERE (period_1.month = 12))))
                  GROUP BY period.period, period.calendar_year, period.month, period.month_long, period.period_date, commodity.product, location.land_type, location.land_class, location.land_category,
                        CASE
                            WHEN ((location.land_class)::text = 'Mixed Exploratory'::text) THEN 0
                            WHEN (((location.land_class)::text = 'Native American'::text) OR ((location.land_class)::text = 'Native American '::text)) THEN 1
                            WHEN (((location.land_class)::text = 'Federal'::text) AND ((location.land_category)::text = 'Onshore'::text)) THEN 3
             ...
             LEFT JOIN ( SELECT DISTINCT period.period,
                  ...
                  WHERE (((period.period)::text = 'Monthly'::text) AND (period.calendar_year <= ( SELECT max(period_1.calendar_year) AS max
                          ...
                          WHERE (period_1.month = 12))) AND (period.calendar_year >= ( SELECT max((period_1.calendar_year - 1)) AS max
                           FROM (public.production production_1
                             JOIN public.period period_1 USING (period_id))
                          WHERE (period_1.month = 12))))) t2 USING (period, year, month, month_long, period_date))) a
...
ALTER TABLE public.total_monthly_calendar_production OWNER TO postgres;
