...
CREATE VIEW public.total_monthly_last_twelve_disbursement AS
 SELECT a.period,
   ...
   FROM ( SELECT period.period,
          ...
          GROUP BY period.period, period.calendar_year, period.month, period.month_long, period.period_date, location.land_type, location.land_class, location.land_category, fund.fund_class, fund.recipient) a
