...
--
-- Name: query_tool_disbursement_data; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.query_tool_disbursement_data AS
 SELECT period.period,
    ...
    sum(
        CASE
            WHEN (period.fiscal_year = 2003) THEN (disbursement.disbursement)::double precision
            ELSE ((0)::real)::double precision
        END) AS y2003,
    sum(
        CASE
            WHEN (period.fiscal_year = 2004) THEN (disbursement.disbursement)::double precision
            ELSE ((0)::real)::double precision
        END) AS y2004,
    sum(
        CASE
            WHEN (period.fiscal_year = 2005) THEN (disbursement.disbursement)::double precision
            ELSE ((0)::real)::double precision
        END) AS y2005,
    sum(
        CASE
            WHEN (period.fiscal_year = 2006) THEN (disbursement.disbursement)::double precision
            ELSE ((0)::real)::double precision
        END) AS y2006,
    sum(
        CASE
            WHEN (period.fiscal_year = 2007) THEN (disbursement.disbursement)::double precision
            ELSE ((0)::real)::double precision
        END) AS y2007,
    sum(
        CASE
            WHEN (period.fiscal_year = 2008) THEN (disbursement.disbursement)::double precision
            ELSE ((0)::real)::double precision
        END) AS y2008,
    sum(
        CASE
            WHEN (period.fiscal_year = 2009) THEN (disbursement.disbursement)::double precision
            ELSE ((0)::real)::double precision
        END) AS y2009,
    sum(
        CASE
            WHEN (period.fiscal_year = 2010) THEN (disbursement.disbursement)::double precision
            ELSE ((0)::real)::double precision
        END) AS y2010,
    sum(
        CASE
            WHEN (period.fiscal_year = 2011) THEN (disbursement.disbursement)::double precision
            ELSE ((0)::real)::double precision
        END) AS y2011,
    sum(
        CASE
            WHEN (period.fiscal_year = 2012) THEN (disbursement.disbursement)::double precision
            ELSE ((0)::real)::double precision
        END) AS y2012,
    sum(
        CASE
            WHEN (period.fiscal_year = 2013) THEN (disbursement.disbursement)::double precision
            ELSE ((0)::real)::double precision
        END) AS y2013,
    sum(
        CASE
            WHEN (period.fiscal_year = 2014) THEN (disbursement.disbursement)::double precision
            ELSE ((0)::real)::double precision
        END) AS y2014,
    sum(
        CASE
            WHEN (period.fiscal_year = 2015) THEN (disbursement.disbursement)::double precision
            ELSE ((0)::real)::double precision
        END) AS y2015,
    sum(
        CASE
            WHEN (period.fiscal_year = 2016) THEN (disbursement.disbursement)::double precision
            ELSE ((0)::real)::double precision
        END) AS y2016,
    sum(
        CASE
            WHEN (period.fiscal_year = 2017) THEN (disbursement.disbursement)::double precision
            ELSE ((0)::real)::double precision
        END) AS y2017,
    sum(
        CASE
            WHEN (period.fiscal_year = 2018) THEN (disbursement.disbursement)::double precision
            ELSE ((0)::real)::double precision
        END) AS y2018,
    sum(
        CASE
            WHEN (period.fiscal_year = 2019) THEN (disbursement.disbursement)::double precision
            ELSE ((0)::real)::double precision
        END) AS y2019,
    sum(
        CASE
            WHEN (period.fiscal_year = 2020) THEN (disbursement.disbursement)::double precision
            ELSE ((0)::real)::double precision
        END) AS y2020
  ...
  GROUP BY period.period, location.state_name,
        CASE
            ...
            WHEN ((location.region_type)::text = 'State'::text) THEN concat('Disbursements to ', location.state_name)
            ELSE NULL::text
        END, fund.recipient, fund.source, period.fiscal_year, period.calendar_year
UNION
 SELECT period.period,
        CASE
            WHEN ((location.state_name)::text <> ''::text) THEN (location.state_name)::text
...
ALTER TABLE public.query_tool_disbursement_data OWNER TO postgres;
