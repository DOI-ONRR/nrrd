            ...
            <article>
              <ul>
                {results.length > 0
                  ...
                  ) : <p><strong>We didn't find any search results for {queryString}.</strong> {(glossaryResults.length > 0) && 
                  <Fragment>You might try searching for <GlossaryTerm termKey={queryString}>{queryString}</GlossaryTerm> in our glossary.</Fragment>}</p>
                }
              </ul>
            </article>
          </div>
