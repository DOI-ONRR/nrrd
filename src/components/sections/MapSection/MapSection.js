...
query DisbursementSummaryQuery {
  onrr {
    revenue(where: {period: {fiscal_year: {_eq: 2018}, period: {_eq: "Fiscal Year"}}}) {
      revenue
      location {
        state
        county
        offshore_planning_area
      }
      period {
        period
      }
...
//    console.debug(options);
    let offshoreJson=withPrefix(props.mapOffshoreJson);
    let mapJson=withPrefix(props.mapJson);
