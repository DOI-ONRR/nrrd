  ...
  useEffect( () => {
    ...
    if(typeof(mapJsonObject) != "object") {
	  ...
	  .then( us => {
	    ...
	    let data=observableData(mapData);
	    data.title=mapTitle;
		...
		.then( offshore => {
		  ...
		  for(let region in  offshore.objects ) {
		    if(ii<1) {
		      offshoreChart(svg,offshore,region,data, offshoreColorScheme ,onClick, minColor,maxColor);
		      //ii++;
		    }
		  }
      ...
      for(let region in  offshore.objects ) {
	offshoreChart(svg,offshore,region,data, offshoreColorScheme ,onClick);
      }
    }
